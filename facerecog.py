...
if os.path.exists('face_compare.jpg'):
	os.unlink('face_compare.jpg')

if os.path.exists('debug_detect_log.txt'):
	os.unlink('debug_detect_log.txt')

if os.path.exists('picasa_compare_response.xml'):
	os.unlink('picasa_compare_response.xml')

for jpg in glob.glob('C:/facerecog/images/*.jpg'):
	os.unlink(jpg)
	
#Capture Face for Comparison for 3 secs
for i in range(0,3):
	...
	capframe.save('face_compare.jpg')
	#sequential file name for comparison photos
	nameCount = str('%d' + '_compare.jpg') %i 
